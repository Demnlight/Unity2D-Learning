...
public class MapGenerator : MonoBehaviour {
    ...
    private void OnDrawGizmos( ) {
        ...
        foreach (var c in chunkManager.GetRenderingChunks( )) {
            Vector3 ChunkPosition = new Vector3( c.Value.vPos.x * ChunkConstants.nChunkSize, c.Value.vPos.y * ChunkConstants.nChunkSize );
            for (int x = 0; x < ChunkConstants.nChunkSize; x++) {
                for (int y = 0; y < ChunkConstants.nChunkSize; y++) {
                    ...
                    float flHeight = c.Value.GetMapHeights[ x, y ];
